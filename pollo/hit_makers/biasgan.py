...
class BiasganHitMakerRace(BiasganHitMaker):
    ...
    def get_processed_data_from_media_folder(self, media_folder):
        media_directory = pjoin(io_utils.get_git_root(os.path.abspath(__file__)), "static/data/media")
        task_question = io_utils.load_from_txt(pjoin(media_directory, media_folder, "task_question.txt"))[0]
        choices = list(pd.read_csv(pjoin(media_directory, media_folder, "choices.csv"))["choices"])
        # populate the examples
        examples = []
        d = pd.read_csv(pjoin(media_directory, media_folder, "examples.csv"))
        ...
        gt_query_examples = []
        d = pd.read_csv(pjoin(media_directory, media_folder, "ground_truth.csv"))
        ...
        for query_folder in os.listdir(pjoin(media_directory, media_folder, "query")):
            for image in os.listdir(pjoin(media_directory, media_folder, "query", query_folder)):
...
class BiasganHitMakerBetter(BiasganHitMaker):
    ...
    def get_processed_data_from_media_folder(self, media_folder):
        ...
        # populate the examples
        examples = []
        d = pd.read_csv(pjoin(media_directory, media_folder, "query_examples.csv"))
        ...
        # d = pd.read_csv(pjoin(media_directory, media_folder, "intersplit_ranking_query.csv"))
...
class BiasganHitMakerBetterTask4(BiasganHitMaker):
    """
    Shown two images (0 or 1), choose the label.
    """
    ...
    def get_processed_data_from_media_folder(self, media_folder):
        ...
        # populate the query examples
        query_examples = []
        filename = pjoin(media_directory, media_folder, "quality_ranking_query.csv")
        d = pd.read_csv(filename)
        for imageA, imageB in zip(d["item1_image_url"], d["item2_image_url"]):
            urlA = os.path.join(self.endpoint, "static/data/media", media_folder, "query", imageA)
            urlB = os.path.join(self.endpoint, "static/data/media", media_folder, "query", imageB)
            urls = [urlA, urlB]
            random.shuffle(urls)
            query_examples.append(self.get_example(
                urls=[urlA, urlB], url_labels=["Image A", "Image B"], choices=choices, answer=None, description=None))

        return {
            "task": "Task4_Quality_Ranking",
            "task_question": task_question,
            "examples": examples,
            "gt_query_examples": gt_query_examples,
...
task_to_class = {
    ...
    "Task4_Quality_Ranking": BiasganHitMakerBetterTask4,
}
