...
class VideoExampleTemplateWithImagePrompts:
    name: str = "VideoExampleTemplateWithImagePrompts"

    def get_div(self, data):
        html_str = ""
        html_str = """<div>"""
        html_str += """<div class="PickNRow">"""
        for i, image_url in enumerate(data["image_prompts"]):
            html_str += f"""<div class="PickNRowItem" width="25%">"""
            html_str += f"""<div class="img-overlay-wrap" width="100%">"""
            html_str += f"""<img height=300 src="{image_url}">"""
            html_str += """</div>"""
            html_str += "</div>"
        html_str += """</div>"""
        html_str += """<div class="PickNRow">"""
...
def get_template_instance(template_name):
    templates = {
        ...
        "VideoExampleTemplateWithImagePrompts": VideoExampleTemplateWithImagePrompts,
    }
